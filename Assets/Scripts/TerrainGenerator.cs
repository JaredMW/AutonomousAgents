...
public class TerrainGenerator : MonoBehaviour
{
    ...
    void CreateWaterPlane()
    {
        ...
        Vector3[] vertices
            = new Vector3[relativeResolution * relativeResolution];
        float xPerRes = (float)Width / (relativeResolution - 1);
        float yPerRes = (float)Length / (relativeResolution - 1);
        ...
        waterObject.transform.position = new Vector3(
            ((float)Width / 2) * transform.lossyScale.x + transform.position.x,
            waterHeightPercentage * terrainData.size.y * transform.lossyScale.y
                + transform.position.y,
            ((float)Length / 2) * transform.lossyScale.z
